...
namespace DBInterface_XUnit_Tests
{
    public class DBManagerTests
    {
        ...
        internal static DBManager build_dynamic_test_mgr() { return DBManager.Build(() => build_dummy()); }
        ...
        {
            [Fact]
            public void AfterDBConnectionChanges_Invoked_SubsequentTo_Change()
            {
                DBManager test = build_dynamic_test_mgr();

                test.XUnit_AftDBCnxCha += (args) =>
                {
                    if (ReferenceEquals(args.OldCnx, args.NewCnx))
                        throw new ApplicationException(); // If this happens, most likely the manager is returning the same instance for every call to its connection-provider

                    Assert.True(ReferenceEquals(args.NewCnx, test.GetCnx_For_XUnit()));
                };

                test.NextConnection(); // Execution moves to DBManagerTests.cs:41 before the instance's connection is updated
            }

            [Fact]
            public void BeforeDBConnectionChanges_Invoked_PriorTo_Change()
            {
                DBManager test = build_dynamic_test_mgr();

                test.XUnit_BefDBCnxCha += (args) =>
                {
                    if (ReferenceEquals(args.OldCnx, args.NewCnx))
                        throw new ApplicationException(); // If this happens, most likely the manager is returning the same instance for every call to its connection-provider
                    
                    Assert.True(ReferenceEquals(args.OldCnx, test.GetCnx_For_XUnit()));
                };
            ...
            [Fact]
            public void AfterDBConnectionChanged_Invoked_During_Call_To_NextConnection()
            {
                IDbConnection newCnx;
                Dummy dummy = build_dummy() as Dummy;
                DBManager test = build_test_mgr(dummy);
                test.XUnit_AftDBCnxCha += set_newcnx_testbit_true;
